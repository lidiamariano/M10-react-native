...
export default function AtualizarDados() {
    ...
    const fetchDatabase = async () => {
        try {
            ...
            // Filtra o usuário específico pelo userId
            ...
            setDatabase(data.filter(user => user.id !== parseInt(userId)));
        } catch (error) {
    ...
    const update = async () => {
        ...
        try {
            const response = await fetch(`http://10.254.19.101:8080/users/${userId}`, {
                method: 'PUT',
                body: formData,
            });
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    errorText = errorData.detail || 'Erro ao mudar as informações do usuário';
                } catch (e) {
            ...
            Alert.alert('Erro', error.message || 'Ocorreu um erro ao atualizar as informações do usuário');
        }
    }
            ...
            >
                <ScrollView>
                    <SafeAreaView style={estilo.fundoContainer}>
                        <Text style={{fontSize: 30, fontWeight: 'bold', color: '#693c3f'}}>Atualize seus dados</Text>
                        <Text style={{fontSize: 20, color: '#693c3f'}}>Preencha os campos que quer atualizar</Text>
                        ...
                        <Botao texto={"Atualizar"} funcao={update}/>
                    </SafeAreaView>
                </ScrollView>
            </KeyboardAvoidingView>
